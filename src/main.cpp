...
static bool tabMenuMoved = false;
static uint32_t tabPressTime = 0;
static uint8_t tabMenuCursor = 0;
...
void loop() {
    ...
    if ((event == INPUT_PLUS || event == INPUT_MINUS) && Input::isFnHeld()
        ...
        if (event == INPUT_PLUS) {
            ...
            displayBrightness = displayBrightness >= 20 ? displayBrightness - 10 : 10;
        }
        M5Cardputer.Display.setBrightness((displayBrightness * 255) / 100);
        brightnessDirty = true;
        brightnessChangeTime = millis();
        char msg[12];
        snprintf(msg, sizeof(msg), "bright %d%%", displayBrightness);
        character.say(msg);
        event = INPUT_NONE;
    }
    ...
    if (event == INPUT_TAB && !tabMenuOpen && currentScreen != SCREEN_PROJECT
        ...
        if (!Input::isTabHeld()) {
            ...
            if (!tabMenuVisible && (millis() - tabPressTime >= TAB_HOLD_THRESHOLD)) {
                tabMenuVisible = true;
            }
    ...
    // Handle project view close/load
    ...
    // Handle pattern select → edit transition
    ...
    // Handle pattern edit → select back
    ...
    // Handle song → pattern edit transition
